function Egg( ctx, x, y ) {
    ...
    this.xPer = 68.5 / 216*width/375;
    this.yPer = 36 / 190*width/375;
    if(width>300&&width<=350){
        this.xPer = 68.5 / 216*width/400;
        this.yPer = 36 / 190*width/395;
    }else if(width>350&&width<=450){
        this.xPer = 68.5 / 216*width/375;
        this.yPer = 36 / 190*width/375;
    }else if(width>450){
        this.xPer = 68.5 / 216*width/300;
        this.yPer = 36 / 190*width/300;
    }

    this.giftX = x + eggRes["width"]*0.45;
    this.giftY = y + eggRes["height"]*0.6;
    ...
    this.draw = function() {
        switch ( this.status ) {
            ...
            case 1:
                ...
                ctx.drawImage( gashRes['image'], ((-gashRes['width']-150)*350/width) * this.xPer, ((-gashRes['height']-150)*350/width) * this.yPer, gashRes['width'], gashRes['height']);
                ctx.translate(  -( this.gashX + gashRes['width'] * this.xPer), -(this.gashY + gashRes['height'] * this.yPer) );
                ctx.restore();
                ctx.drawImage( giftRes['image'], this.giftX, this.giftY, giftRes['width'], giftRes['height'] );
                break;
